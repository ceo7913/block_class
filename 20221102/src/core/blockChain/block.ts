...
// IBlock 형태로 클래스를 선언, 가져오고 BlockHeader 를 상속 받은 Block
export class Block extends BlockHeader implements IBlock {
    ...
    constructor(_previousBlock : Block , _data : string[], _adjustmentBlock : Block){
        ...
        // nonce 값은 마이닝에서 0의 갯수를 난이도에 맞춰서 찾을때 까지 증가시킬 값
        // 가장 많이 사용하는 해시 알고리즘인 SHA256 으로 임의의 데이터를 해시한 값을 구해서
        // 256bit(32바이트) 의 값을 얻는것
        // 해시함수는 단방향 함수로 데이터로부터 해시값을 구할 수는 있는데
        // 해시값으로 데이터를 역산할 수는 없다. 원본 데이터를 알 수 가 없다는 것 
        // 해시값이 같은 임의의 데이터를 만들어 내는건 정말 어렵고 (경우의 수를 다찾는데)
        // 특정한 해시값이 나오는 데이터를 찾으려면 256개 (256비트로 나올 수 있는 모든 경우의 수)의 
        // 데이터를 다 확인하는데 지금 우리 컴퓨터로는 불가능한 작업이다.
        ...
        // 초기값 설정
        // 첫번째 초기값은 실제로 결과가 될 값 (변하는 도중임)
        // 첫번째 초기값은 256bit 로 총 8개의 32bit 값으로 이루어지게 되는데

        // 가장 작은 8개의 소수의 제곱근
        // [2,3,5,7,11,13,17,19] 의 제곱근
        // 이 배열을 반복문으로 돌려서 제곱근을 구하고 정수값을 제거하고 왼쪽으로 32bit shift 연산 후
        // 16진수로 출력하면
        // KISA 에서 제공하는 SHA256 공식 소스 코드의 고정된 초기값
        // 0x6a09e667
        // 0xbb67ae85
        // 0x3c6ef372
        // 0xa54ff53a
        ...
        // 3제곱근을 구하고 정수 값을 제거 후 왼쪽으로 32bit shift 연산 후 16 진수로 출력하면
        // 0x428a2f98
        // 0x71374491
        // 0xb5c0fbcf
        // 0xe9b5dba5
        ...
        this.nonce = 0; 
        // getDifficulty 함수로 난이도를 생성한것
        // 블록의 생성 시간을 조절해 주는것 블록이 생성되는 시간을 10 마다 비교해서
        // 시간을 조절해 주기 위해서 사용하는것
    ...
    public static getDifficulty(_newBlock : Block, _adjustmentBlock : Block, _previousBlock:Block) : number{ 
        // 추가된 블록의 높이가 9 이거나 작으면 난이도를 0 고정시키고 
        if(_newBlock.height <= 9) return 0;
        // 추가된 블록의 높이가 10 이거나 작으면 난이도를 1
        if(_newBlock.height <= 10) return 1;

        // 10번째 배수의 블록에 한해서만 난이도 구현
        // 10개의 묶음씩 같은 난이도를 가지게 한다.
        // 새 블록의 높이를 나머지 연산으로 10으로 나눠서 나머지가 0이 아니면 
        // 이전 블록의 난이도를 보내주고 (20번째 블록 전까지 난이도가 이전블록 난이도인 1 이라는 것) 
        ...
        // 난이도 조절 식
        // timeTaken 이 차이값을 구한 시간이 블록의 생성 주기 시간 / 2 보다 작으면
        // 난이도를 1 올려주고 
        // timeTaken 이 차이값이 블록 생성 주기 시간 * 2 보다 크면 난이도를 1 내려준다.
        // 이렇게 해서 블록의 난이도를 조절해서 블록의 생성 주기를 조절해 주는것
    ...
    public static findBlock(generateBlock : Block) {
        ...
        while(true)
        {
            ...
            nonce++;
            // nonce 가 올려진 블록 해시를 createBlockHash 함수로 블록의 모든 내용을 해시화 해서
            // 해시값을 얻고
    ...
    public static getMerkleRoot<T>(_data:T[]) : string {
        // 머클트리를 만들어주는 라이브러리 merkle sha256 알고리즘으로 해싱을 하고 
        // sync 함수에 매개변수로  _data  배열을 전달해서 
